...
const lineColor = (opacity, props) =>
  props.secondary
    ? rgba(props.theme.textColor, opacity)
    : rgba(props.theme.accentColor, opacity);
...
const Anchor = styled.a.attrs(({ target, rel }) => ({
  ...
  color: ${(props) => (props.secondary ? "inherit" : props.theme.accentColor)};

  background: linear-gradient(
        ${(props) => lineColor(1, props)},
        ${(props) => lineColor(1, props)}
      )
      no-repeat 100% 100% / 0 2px,
    linear-gradient(
        ${(props) => lineColor(0.3, props)},
        ${(props) => lineColor(0.3, props)}
      )
